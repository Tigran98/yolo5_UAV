...
import val_fusion as validate  # for end-of-epoch mAP (fusion model with dual inputs)
...
def train(hyp, opt, device, callbacks):
    ...
    if pretrained:
        ...
        # Load weights into both RGB and IR backbones
        model_dict = model.state_dict()
        loaded_count = 0
        
        # Map standard YOLOv5 weights to dual backbone format
        # Pretrained format: model.{i}.* (backbone layers 0-9, head layers 10+)
        # Fusion format: rgb_backbone.{i}.*, ir_backbone.{i}.*, head.{i-10}.*
        backbone_len = 10  # YOLOv5n backbone has 10 layers
        
        for k, v in csd.items():
            if k.startswith('model.'):
                layer_part = k[6:]  # Remove 'model.'
                try:
                    layer_idx = int(layer_part.split('.')[0])
                    
                    if layer_idx < backbone_len:
                        # Backbone layer - map to both RGB and IR backbones
                        # Extract the rest of the key after layer index
                        parts = layer_part.split('.', 1)
                        if len(parts) > 1:
                            rest = parts[1]
                            k_rgb = f'rgb_backbone.{layer_idx}.{rest}'
                            k_ir = f'ir_backbone.{layer_idx}.{rest}'
                            
                            # Load into RGB backbone
                            if k_rgb in model_dict and v.shape == model_dict[k_rgb].shape:
                                model_dict[k_rgb] = v.clone()
                                loaded_count += 1
                            
                            # Load into IR backbone (same weights)
                            if k_ir in model_dict and v.shape == model_dict[k_ir].shape:
                                model_dict[k_ir] = v.clone()
                                loaded_count += 1
                    else:
                        # Head layer - map to fusion model head
                        head_idx = layer_idx - backbone_len
                        parts = layer_part.split('.', 1)
                        if len(parts) > 1:
                            rest = parts[1]
                            k_head = f'head.{head_idx}.{rest}'
                            
                            if k_head in model_dict and v.shape == model_dict[k_head].shape:
                                model_dict[k_head] = v.clone()
                                loaded_count += 1
    ...
    # Trainloader
    train_loader, dataset = create_fusion_dataloader(
        train_path,
        imgsz,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (rgb_imgs, ir_imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if i == 0 and epoch == 0:
                ...
                LOGGER.info(f"DEBUG: targets (first 5): {targets[:5] if len(targets) > 0 else 'EMPTY'}")
                LOGGER.info(f"DEBUG: Number of targets: {len(targets)}")
                if len(targets) > 0:
                    LOGGER.info(f"DEBUG: Target classes: {targets[:, 1].unique()}")
                    LOGGER.info(f"DEBUG: Target image indices: {targets[:, 0].unique()}")
