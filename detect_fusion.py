...
def run(
    ...
    names = model.names if hasattr(model, 'names') else {0: 'drone'}
    imgsz = check_img_size(imgsz, s=stride)
    
    # Check for webcam/screenshot (not supported for fusion model)
    if webcam or screenshot:
        LOGGER.warning("Webcam and screenshot modes not supported for fusion model. Using image directory.")
        webcam = False
        screenshot = False
